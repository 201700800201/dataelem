...
def convert_label(json_file, outpath, imagepath):
    annos = json.load(open(json_file))
    # ip.set_trace()
    ...
    for anno in tqdm(annos): 
        ...
        for i, info in enumerate(bbox_infos):
            ...
            if info.get('type') == 'labels':
                ...
                tmp_dict['value'] = info.get('value').get('text')[0]
    ...
    for valimage in val_image:
        for filetype in filetypes:
            if os.path.exists(os.path.join(imagepath, valimage+filetype)):
                shutil.copy(os.path.join(imagepath, valimage+filetype), os.path.join(outpath, 'Images/val'))
                break
      
    for trainimage in train_image:
        for filetype in filetypes:
            if os.path.exists(os.path.join(imagepath, trainimage+filetype)):
                shutil.copy(os.path.join(imagepath, trainimage+filetype), os.path.join(outpath, 'Images/train'))
                break
...
def main():
    ...
    for label in os.listdir(args.data_dir):
        if isinstance(label, str) and not label.endswith(".json"):
            continue
        if os.path.isdir(os.path.join(args.data_dir, label)):
            continue
        output_path = os.path.join(args.output_file, label.split('.')[0])
        if not os.path.exists(output_path):
            os.mkdir(output_path)
            os.chdir(output_path)
        download_images(os.path.join(args.data_dir, label), '../download_images')
        convert_label(os.path.join(args.data_dir, label), output_path, '../download_images')
